...
struct Config
{
  ...
  // 중앙 주행 기준선. 예전에는 위 두 값의 평균을 썼는데, 두 값이 0.5 를 중심으로
  // 대칭이 아니면(0.63/0.35 → 0.49) 중앙 주행이 조용히 한쪽으로 치우친다.
  // 독립된 값으로 두어 좌/우 차선 기준과 따로 조정할 수 있게 한다.
  float center_reference_center;      // 중앙 주행 시 기준선 x 비율
