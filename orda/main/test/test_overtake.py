...
def test_completes_only_after_pass_delay():
    ...
    seen = g.update_zone(now=2.0, lane_target=1, side_left=INF, side_right=0.42)
...
def test_side_just_seen_fires_once():
    g = guard()
    g.enter_zone(now=0.0)
    first = g.update_zone(now=0.1, lane_target=1, side_left=INF, side_right=0.4)
    second = g.update_zone(now=0.2, lane_target=1, side_left=INF, side_right=0.4)
...
def test_measured_lane_wins_over_lane_target_for_side_choice():
    ...
    g.enter_zone(now=0.0)
    assert g.update_zone(
        now=0.1, lane_target=2, side_left=INF, side_right=0.4, ego_lane=1
    ).side_just_seen is True
    ...
    assert g2.update_zone(
        now=0.2, lane_target=2, side_left=0.4, side_right=INF, ego_lane=-1
    ).side_just_seen is True
...
def test_reset_and_enter_zone_clear_state():
    g = guard()
    g.enter_zone(now=0.0)
    g.update_zone(now=0.1, lane_target=1, side_left=INF, side_right=0.4)
